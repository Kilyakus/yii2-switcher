...
{
	$(target).switcher({copy: {en: {yes: '', no: ''}}}).on('change', function(){
		var checkbox = $(this);
		if(checkbox.data('link')){
			checkbox.switcher('setDisabled', true);

			var link = checkbox.data('link') + 
						(checkbox.data('sublink') ? '/'+checkbox.data('sublink') : '/') +
						(checkbox.is(':checked') ? 'on' : 'off') + 
						(checkbox.data('id') ? '/' + checkbox.data('id') : '');
